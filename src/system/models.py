...
class Users(CoreModel, AbstractUser):
    ...
    openid = models.CharField(max_length=150, unique=True, db_index=True,
                              null=True, blank=True, verbose_name="openid", help_text="openid")
...
class Role(CoreModel):
    ...
    remark = models.TextField(
        verbose_name="备注", help_text="备注", null=True, blank=True)
    ...
    def recursion_dept_info(cls, dept_id: int, dept_all_list=None, dept_list=None):
        ...
        for ele in dept_all_list:
            if ele.get("parent") == dept_id:
                ...
                cls.recursion_dept_info(
                    ele.get("id"), dept_all_list, dept_list)
...
class MenuButton(CoreModel):
    ...
    value = models.CharField(
        max_length=64, verbose_name="权限值", help_text="权限值")
...
class MessageCenter(CoreModel):
    ...
    target_user = models.ManyToManyField(to=Users, related_name='user', through='MessageCenterTargetUser', through_fields=(
        'messagecenter', 'users'), blank=True, verbose_name="目标用户", help_text="目标用户")
...
class MessageCenterTargetUser(CoreModel):
    users = models.ForeignKey(Users, related_name="target_user", on_delete=models.CASCADE,
                              db_constraint=False, verbose_name="关联用户表", help_text="关联用户表")
    messagecenter = models.ForeignKey(MessageCenter, on_delete=models.CASCADE,
                                      db_constraint=False, verbose_name="关联消息中心表", help_text="关联消息中心表")
